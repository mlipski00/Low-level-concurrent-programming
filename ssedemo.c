...
int wyraz(float *a,float *b,float *c,int n,float *wyn,int typAdr){
    ...
    if(typAdr == 1){ // wersja pierwsza z adresowaniem argumentów
                     // dla SSE za pomoc¹ skalarnych zmiennych
                     // wskaŸnikowych
        ...
        for (i = 0; i < n / 4; i++) {
            ...
            wynRef += 4;
                    ...
                    // za pomoc¹ adresów elementów tablic
    {
        __m128 asse, bsse, csse;
...
int main(int argc, char* argv[]){
    ...
    b = _mm_malloc(n * sizeof(float), 16);
                           // adresu pocz¹tkowego do granic bloków
    c = _mm_malloc(n * sizeof(float), 16);
                           // 16-bajtowych (tego wymagaj¹ operacje
